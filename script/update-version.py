...
table = (
    ...
    , ( 'CMakeLists.txt'
        , r'set\W+observer_ptr_version\W+"([0-9]+\.[0-9]+\.[0-9]+)"\W+$'
        , 'set( observer_ptr_version "{major}.{minor}.{patch}" )\n' )

#    , ( 'example/cmake-pkg/CMakeLists.txt'
#        , r'set\W+observer_ptr_version\W+"([0-9]+\.[0-9]+(\.[0-9]+)?)"\W+$'
#        , 'set( observer_ptr_version "{major}.{minor}" )\n' )
#
...
#        , 'observer_ptr_version = "{major}.{minor}.{patch}"\n' )
    ...
    , ( 'include/nonstd/observer_ptr.hpp'
        , r'\#define\s+observer_ptr_MINOR\s+[0-9]+\s*$'
        , '#define observer_ptr_MINOR  {minor}' )

    , ( 'include/nonstd/observer_ptr.hpp'
        , r'\#define\s+observer_ptr_PATCH\s+[0-9]+\s*$'
        , '#define observer_ptr_PATCH  {patch}\n' )
)
